...
//        holder.txtExpiryDate.setText(cardDetails.get(position).getExpiryMonth() + "/" + cardDetails.get(position).getExpiryDate());
...
//    }
    ...
    @Override
    public void onBindViewHolder(@NonNull final PaymentViewHolder holder, int position) {
        String cardNumber = cardDetails.get(position).getCardNumber();
        ...
        // Mask with stars + show last 4
        String maskedCard = "**** **** **** " + lastFour;

        holder.txtCardNumber.setText(maskedCard);
        holder.txtExpiryDate.setText(cardDetails.get(position).getExpiryMonth() + "/" + cardDetails.get(position).getExpiryDate());
        holder.txtNameOnCard.setText(cardDetails.get(position).getCardHolderName());

        if ("1".equals(cardDetails.get(position).getIsDefault())) {
            holder.chkDefault.setChecked(true);
            holder.btnDefault.setVisibility(View.VISIBLE);
        } else {
            holder.chkDefault.setChecked(false);
