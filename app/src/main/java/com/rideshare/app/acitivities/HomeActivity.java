...
import com.rideshare.app.pojo.StripeResponse;
...
public class HomeActivity extends ActivityManagePermission implements NavigationView.OnNavigationItemSelectedListener, ProfileFragment.ProfileUpdateListener, ProfileFragment.UpdateListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        BindView();
        getStripekey(getApplicationContext());
...
//        checkForUpdates();
    ...
    // This is Call after click on side menu
    @Override
    ...
    public void showAlert(Context context, String title, String message) {
        new AlertDialog.Builder(context, AlertDialog.THEME_HOLO_DARK).setMessage(message).setCancelable(false)
    ...
    public void deleteAccount() {
        ...
        if (CheckConnection.haveNetworkConnection(this)) {
            ...
            Call<DeleteAccount> call = apiService.deleteAccount("Bearer " + SessionManager.getKEY());
    ...
    public void logout() {
        ...
        if (CheckConnection.haveNetworkConnection(this)) {
            ...
            Call<LogoutResponse> call = apiService.logoutStatus("Bearer " + SessionManager.getKEY(), logoutStatus);
    ...
    public void update(String url) {
        if (!url.equals("")) {
            Glide.with(getApplicationContext()).load(url).apply(new RequestOptions().error(R.drawable.images).placeholder(R.drawable.ridesharelogo)).into(avatar);
        }
    }
    ...
    private void getProfile() {
        ...
        call.enqueue(new Callback<GetProfile>() {
            ...
            public void onResponse(Call<GetProfile> call, retrofit2.Response<GetProfile> response) {
                ...
                Log.d("profileResponse",response.body().getData().toString());
                if (jsonResponse != null) {
                    if (jsonResponse.getStatus()) {
                        ...
                        if (jsonResponse.getData().getProfileImage() != null) {
                            ...
                            if (profileUrl.isEmpty() || profileUrl.equalsIgnoreCase("")) {
                                ...
                                Glide.with(HomeActivity.this).load(jsonResponse.getData().getProfilePic()).listener(new RequestListener<Drawable>() {
                                    @Override
            ...
            public void onFailure(Call<GetProfile> call, Throwable t) {
                Log.d("ProfileFailed", "RetrofitFailed");
            }
...
//            param.put("app_version", BuildConfig.VERSION_NAME);
            ...
            Call<CheckAppVersionResponse> call = apiService.checkAppVersionApi("Bearer " + SessionManager.getKEY(), param);
    ...
    // To check for app update from play store
    private void checkForUpdates() {
        try {
            appUpdateManager.getAppUpdateInfo().addOnSuccessListener(appUpdateInfo -> {
                if (appUpdateInfo.updateAvailability() == UpdateAvailability.UPDATE_AVAILABLE) {
                    // Prompt the user to update
                Log.d("APP_UPDATE_INFO1", appUpdateInfo + "");
                    showUpdateDialog();
                }
            });

            appUpdateManager.getAppUpdateInfo().addOnFailureListener(e -> {
                Log.d("APP_UPDATE_INFO2", e.getMessage() + "");
            });
        }catch (Exception e){
            Log.e("APP_UPDATE_INFO3", e.getMessage() + "");
        }
    }

    // API to fetch Strip Key from Server
    public static void getStripekey(Context context) {
        if (CheckConnection.haveNetworkConnection(context)) {
            ApiNetworkCall apiService = ApiClient.getApiService();
            Call<StripeResponse> call = apiService.paymentgatwaykey("Bearer " + SessionManager.getKEY());
            call.enqueue(new Callback<StripeResponse>() {
                @Override
                public void onResponse(Call<StripeResponse> call, Response<StripeResponse> response) {
                    Log.d("logStripekeyRes", response + "");
                    StripeResponse jsonResponse = response.body();
                    if (jsonResponse.getStatus()) {
                        Server.SPIKE_KEY = jsonResponse.getStripe_pubish_key();
                        Log.d("logStripekeyKey", Server.SPIKE_KEY + "");
                    }
                }

                @Override
                public void onFailure(Call<StripeResponse> call, Throwable throwable) {
                    Log.d("logStripekeyFail", throwable + "");
                }
